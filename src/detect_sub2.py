...
def draw_cones(image, cones):
    if len(cones) > 1:
        for i in range(len(cones) - 1):
            if i + 1 < len(cones):
                center_point1 = (int((cones[i].xmin + cones[i].xmax) * 0.5), cones[i].ymax)
                center_point2 = (int((cones[i + 1].xmin + cones[i + 1].xmax) * 0.5), cones[i+1].ymax)
                if cones[0].Class == 'yellow_cone':
                    cv2.line(image, center_point1, center_point2, (0, 255, 255), 10)
                else:
                    cv2.line(image, center_point1, center_point2, (255, 0, 0), 10)
    
    elif len(cones) == 1:
        point = (int((cones[0].xmin + cones[0].xmax) * 0.5), cones[0].ymax)
